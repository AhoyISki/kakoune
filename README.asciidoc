...
content inside. The content is printed by outputing every line that has 
changed, prepended with a `\x1b[\{line\}H` in order to place each line on the 
correct row. To terminate a line, the standard `\x1b[K` sequence is called when 
...
the rate at which kakoune was printing), the performance drops to the expected 
